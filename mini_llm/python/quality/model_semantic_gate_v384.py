...
"""
Semantic Gate V3.8.4 per mini LLM.
...
GATE_NAME = "model_semantic_gate_v384"
OUT_DIR = Path("mini_llm/data/quality/model_semantic_gate_v384")
REPORT_PATH = Path("mini_llm/reports/model_semantic_gate_v384_report.md")
...
def write_outputs(results: List[Result], summary: Dict[str, Any]) -> None:
    ...
    (OUT_DIR / "model_semantic_gate_v384_results.json").write_text(
        json.dumps(serializable, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    (OUT_DIR / "model_semantic_gate_v384_manifest.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    lines: List[str] = []
    lines.append("# Model Semantic Gate V3.8.4 Report")
    lines.append("")
