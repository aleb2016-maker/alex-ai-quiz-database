...
"""
Inference Engine V3.14 - Rewrite Only Safe Decoder.
...
ENGINE_NAME = "inference_engine_v314_rewrite_only_safe_decoder"
GENERATION_MODE = "rewrite_only_safe_semantic_decoder_v314"

OUTPUT_DIR = Path("mini_llm/data/inference_v314_rewrite_only_safe_decoder")
REPORT_PATH = Path("mini_llm/reports/inference_engine_v314_rewrite_only_safe_decoder_report.md")
...
def source_is_compatible(prompt: str, source: str) -> Tuple[bool, List[str], float]:
    ...
    if "il phishing è una tecnica" in low and p not in {"phishing", "social engineering"}:
...
def safe_extractive_rewrite(prompt: str, source: str) -> Optional[str]:
    """
    V3.14:
    vieta copia identica.
    Accetta solo riscritture conservative con soggetto esplicito.
    Se non sa riscrivere bene, fallisce.
    """
    p = normalize_text(prompt).lower()
    ...
    # Questo è il punto principale rispetto a V3.13.
...
def write_outputs(items: Sequence[GeneratedItem], corpus_size: int) -> None:
    ...
    outputs_path = OUTPUT_DIR / "inference_engine_v314_rewrite_only_safe_decoder_outputs.json"
    manifest_path = OUTPUT_DIR / "inference_engine_v314_rewrite_only_safe_decoder_manifest.json"
    ...
    manifest = {
        ...
        "acceptance_rule": "V3.14 non richiede 28/28; richiede qualità alta, almeno 10 OK, zero copie identiche e zero score negativi.",
    }

    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")

    lines: List[str] = []
    lines.append("# Inference Engine V3.14 Safe Dynamic Decoder Report")
    lines.append("")
...
def main() -> int:
    ...
    summary = {
        ...
        "ok": ok,
        "failed": len(items) - ok,
        "outputs_path": str(OUTPUT_DIR / "inference_engine_v314_rewrite_only_safe_decoder_outputs.json"),
