...
MOTORI = {
    "scienze": {
        "nome": "Scienze generali",
        "file": ROOT / "data" / "scienze.json",
        "slug": "scienze",
    },
    "biologia": {
        "nome": "Biologia",
        "file": ROOT / "data" / "biologia.json",
        "slug": "biologia",
    },
    "fisica": {
        "nome": "Fisica",
        "file": ROOT / "data" / "fisica.json",
        "slug": "fisica",
    },
    "chimica": {
        "nome": "Chimica",
        "file": ROOT / "data" / "chimica.json",
        "slug": "chimica",
    },
    "fisica_quantistica": {
        "nome": "Fisica quantistica",
        "file": ROOT / "data" / "fisica_quantistica.json",
        "slug": "fisica_quantistica",
    },
}
...
def crea_file_lavoro(materia):
    if materia not in MOTORI:
        nomi = ", ".join(MOTORI.keys())
        raise SystemExit(
            f"ERRORE: materia non riconosciuta: {materia}\n"
            f"Materie disponibili: {nomi}"
        )

    info = MOTORI[materia]
    nome_motore = info["nome"]
    input_file = info["file"]
    slug = info["slug"]

    output_json = ROOT / "data" / "revisioni" / f"{slug}_domande_da_migliorare.json"
    output_md = ROOT / "reports" / f"{slug}_distrattori_da_migliorare.md"
    ...
    righe = []
    righe.append(f"# {nome_motore} — distrattori da migliorare")
    righe.append("")
    righe.append(f"Domande totali in `{input_file.relative_to(ROOT)}`: **{len(domande)}**")
    ...
    print(f"----- ESTRAZIONE {nome_motore.upper()} DA MIGLIORARE -----")
    ...
    if len(domande_da_migliorare) == 0:
        print(f"OK: nessuna domanda di {nome_motore} da migliorare.")
    else:
        print(f"OK: file di lavoro {nome_motore} creati.")
...
def main():
    if len(sys.argv) < 2:
        nomi = ", ".join(MOTORI.keys())
        raise SystemExit(
            "ERRORE: devi indicare la materia.\n"
            f"Esempio: python3 scripts/estrai_motore_scientifico_da_migliorare.py chimica\n"
            f"Materie disponibili: {nomi}"
        )

    materia = sys.argv[1].strip().lower()
    crea_file_lavoro(materia)
