...
orchestrator = ROOT / "demo-rag" / "rag-summary-universal-orchestrator-v2a34u.js"
universal = ROOT / "demo-rag" / "universal-document-learning-engine.js"
html = ROOT / "demo-rag" / "test-documenti-universale.html"

report_dir = ROOT / "reports" / "orchestratore_riassunto_universale_v2a34u"
backup_dir = report_dir / "backups"
backup_dir.mkdir(parents=True, exist_ok=True)

ts = datetime.now().strftime("%Y%m%d_%H%M%S")

for p in [orchestrator, universal, html]:
    if p.exists():
        b = backup_dir / f"{p.name}.{ts}.bak"
        b.write_text(p.read_text(encoding="utf-8"), encoding="utf-8")
        print(f"Backup creato: {b}")

ORCHESTRATOR_JS = r'''(function (root, factory) {
  const api = factory();
...
orchestrator.write_text(ORCHESTRATOR_JS, encoding="utf-8")
print(f"Creato pacchetto universale: {orchestrator}")

# Aggancio nel motore universale, senza toccare vecchi motori.
txt = universal.read_text(encoding="utf-8")

helper_marker = "/* V2A34U - ponte orchestratore riassunto universale */"
...
'''
    needle = "  async function generaRiassunto()"
    if needle not in txt:
      raise SystemExit("ERRORE: non trovo generaRiassunto per inserire il ponte V2A34U")

    txt = txt.replace(needle, helper + "\n" + needle, 1)

# Forza let su sezioniFinali se ancora const.
txt = txt.replace("    const sezioniFinali =", "    let sezioniFinali =", 1)

call_marker = "applicaOrchestratoreRiassuntoUniversaleV2A34U("

if call_marker not in txt:
    render_line = "    renderizzaRiassuntoLungoV2A28(profilo, progressive, sezioniFinali, qualita.report);"
    if render_line not in txt:
        raise SystemExit("ERRORE: non trovo renderizzaRiassuntoLungoV2A28 per agganciare V2A34U")

    call_block = '''    sezioniFinali = applicaOrchestratoreRiassuntoUniversaleV2A34U(
      testo,
      profilo,
      reportLungo,
      progressive,
...
'''
    txt = txt.replace(render_line, call_block + render_line, 1)

universal.write_text(txt, encoding="utf-8")
print("Ponte V2A34U inserito nel motore universale senza modificare i motori esistenti.")

# HTML: carica il pacchetto universale prima del motore principale. Rimuove eventuale vecchio V2A34 non universale.
html_txt = html.read_text(encoding="utf-8")

html_txt = re.sub(
    r'\s*<script src="\./rag-summary-high-quality-orchestrator-v2a34\.js\?v=[^"]+"></script>\n?',
    "\n",
    html_txt
)

script = '<script src="./rag-summary-universal-orchestrator-v2a34u.js?v=v2a34u-universal"></script>'

if script not in html_txt:
    m = re.search(r'(<script src="\./universal-document-learning-engine\.js\?v=[^"]+"></script>)', html_txt)
    if not m:
        raise SystemExit("ERRORE: non trovo universal-document-learning-engine.js nella pagina")

    html_txt = html_txt[:m.start()] + script + "\n" + html_txt[m.start():]

html_txt = re.sub(
    r'\./universal-document-learning-engine\.js\?v=[^"]+',
    './universal-document-learning-engine.js?v=v2a34u-universal-bridge',
    html_txt,
    count=1
)

html.write_text(html_txt, encoding="utf-8")
print("Pacchetto universale V2A34U collegato alla pagina senza modifiche grafiche.")

report = report_dir / "report_orchestratore_riassunto_universale_v2a34u.md"
report.write_text(
    "# V2A34U - Orchestratore riassunto universale\n\n"
    "Motori aggiunti:\n"
    "1. Copertura universale fino a 500 pagine.\n"
    "2. Estrazione concetti universale.\n"
    "3. Fusione concettuale universale.\n"
    "4. Anti-template e riscrittura macro-sezioni universale.\n"
    "5. Quality gate universale.\n\n"
    "Vincoli rispettati:\n"
    "- Non modificati motori esistenti.\n"
    "- Non modificati pulsanti.\n"
    "- Non modificata grafica.\n"
    "- Non modificati PDF, card, test, domande studio.\n"
    "- Il pacchetto lavora dopo i motori già esistenti e prima del render del riassunto.\n\n"
    "Dominio supportato:\n"
    "- scientifico, tecnico, artistico, narrativo, divulgativo/didattico, giuridico/amministrativo, aziendale, personale, sportivo, generico.\n",
    encoding="utf-8"
)

print("OK V2A34U: orchestratore universale installato.")
