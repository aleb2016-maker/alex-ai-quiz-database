...
# -*- coding: utf-8 -*-
"""
Patch chirurgica RAG V2A30B - riassunti profile-aware.
...
Crea backup automatici in reports/fix_riassunto_profile_aware_v2a30b/backups/
"""
...
REPORT_DIR = ROOT / "reports" / "fix_riassunto_profile_aware_v2a30b"
...
def replace_regex(text: str, pattern: str, replacement: str, label: str) -> str:
    # Usa una lambda per evitare che Python interpreti backslash JS tipo \u0300
    # come escape della replacement regex. La replacement deve essere testo letterale.
    regex = re.compile(pattern, flags=re.S | re.M)
    new_text, count = regex.subn(lambda _match: replacement, text, count=1)
    if count != 1:
...
def write_report() -> None:
    ...
    report = REPORT_DIR / "report_fix_riassunto_profile_aware_v2a30b.md"
    report.write_text(
