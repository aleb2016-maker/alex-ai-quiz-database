...
def formatta_fumetto_dialoghi(testo: str) -> str:
    """
    Formatter fumetto completo:
    - separa dialoghi/balloon
    - conserva testi decorativi/cartelli/striscioni/lavagne
    - evita di mischiare tutto in una sola frase
    """
    testo = pulisci_testo(testo)
    ...
    correzioni = [
        ...
        (r"\brelat\b", "relax"),
        (r"\bpaesan\b", "pausa"),
        (r"\bin paesan\b", "già in pausa"),
        (r"\bgia in pausa\b", "già in pausa"),
    ]
    ...
    def aggiungi(lista, frase):
        frase = re.sub(r"\s+", " ", frase).strip(" -|.,;:")
        if frase and frase not in lista:
            lista.append(frase)

    # 1. Estrazione testi decorativi/cartelli/striscioni
    decorativi_regex = [
        (r"buona festa dei lavoratori", "Buona Festa dei Lavoratori"),
        (r"rock\s*&\s*relax", "Rock & Relax"),
        (r"lavorare stanca.*?non lavorare.*?ringiovanisce!?", "Lavorare stanca, non lavorare ringiovanisce!"),
        (r"ferie in corso", "Ferie in corso"),
        (r"direttore del dolce far niente", "Direttore del dolce far niente"),
        (r"non disturbare", "Non disturbare"),
        (r"già in pausa|gia in pausa", "Già in pausa"),
    ]

    for pattern, frase in decorativi_regex:
        if re.search(pattern, t, flags=re.I):
            aggiungi(decorativi, frase)

    # 2. Estrazione balloon/dialoghi
    if re.search(r"oggi pausa strategica", t, flags=re.I):
        aggiungi(balloon, "Oggi pausa strategica.")

    if re.search(r"per forza.*disoccupati.*mestiere faticoso", t, flags=re.I):
        aggiungi(balloon, "Per forza essere disoccupati tutto l'anno è un mestiere faticoso.")

    # 3. Fallback per altri fumetti/dialoghi diversi
    if not balloon:
        pezzi = re.split(r"(?<=[.!?])\s+", t)

        parole_dialogo = [
            "ciao", "hai visto", "sì", "si", "eccola", "pantheon",
            "perfetto", "andiamo", "ottima idea", "facciamo",
            "mappa", "ristorante", "passeggiata", "mi chiamo",
            "guarda", "aiuto", "aspetta"
        ]
        ...
        for p in pezzi:
            ...
            contiene_dialogo = any(k in p.lower() for k in parole_dialogo)
            ha_punteggiatura = any(x in p for x in [".", "!", "?"])

            if contiene_dialogo or ha_punteggiatura:
                ...
                # Se è tutto maiuscolo probabilmente è cartello, non balloon
                if rapporto_maiuscole > 0.75 and not contiene_dialogo:
                    continue

                aggiungi(balloon, p[0].upper() + p[1:])

    # 4. Se non c'è niente, ritorna vuoto
    if not balloon and not decorativi:
        return ""
    ...
    if balloon:
        sezioni.append(
            "Dialoghi/balloon estratti:\n" +
            "\n".join(f"- {frase}" for frase in balloon)
        )

    if decorativi:
        sezioni.append(
            "Testi decorativi/cartelli/striscioni:\n" +
            "\n".join(f"- {frase}" for frase in decorativi)
        )
