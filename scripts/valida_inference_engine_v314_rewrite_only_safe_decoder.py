...
"""
Validatore V3.14 Rewrite Only Safe Decoder.
...
def main() -> int:
    ...
    inference = root / "mini_llm/python/inference/inference_engine_v314_rewrite_only_safe_decoder.py"
    gate = root / "mini_llm/python/quality/model_semantic_gate_v386.py"
    outputs = root / "mini_llm/data/inference_v314_rewrite_only_safe_decoder/inference_engine_v314_rewrite_only_safe_decoder_outputs.json"
    gate_input = root / "mini_llm/data/inference_v314_rewrite_only_safe_decoder/inference_engine_v314_rewrite_only_safe_decoder_gate_input.json"
    manifest = root / "mini_llm/data/quality/model_semantic_gate_v386/model_semantic_gate_v386_manifest.json"
    validation_report = root / "mini_llm/reports/validazione_inference_engine_v314_rewrite_only_safe_decoder.md"
    ...
    if not outputs.exists():
        print(f"ERRORE: output V3.14 non generato: {outputs}")
        return 2
    ...
    lines = []
    lines.append("# Validazione Inference Engine V3.14 Rewrite Only Safe Decoder")
    lines.append("")
    ...
    lines.append("## Regola")
    lines.append("")
    lines.append("V3.14 è accettabile solo se:")
    lines.append("")
