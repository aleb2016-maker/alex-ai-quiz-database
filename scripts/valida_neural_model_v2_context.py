...
class NeuralModelV2ContextValidator:
    """
    Validatore Neural Model V2 Context.

    Controlla:
    - file modello presenti;
    - manifest coerente;
    - architettura multi-token attiva;
    - pesi leggibili;
    ...
    def validate(self, root: Path) -> List[str]:
        ...
        model_dir = root / "mini_llm" / "data" / "model_v2_context"
        report_dir = root / "mini_llm" / "reports"

        weights_path = model_dir / "neural_model_v2_context_weights.json"
        manifest_path = model_dir / "neural_model_v2_context_manifest.json"
        predictions_path = model_dir / "neural_model_v2_context_sample_predictions.json"
        report_path = report_dir / "neural_model_v2_context_report.md"
    ...
    def _validate_manifest(self, manifest: Dict, errors: List[str]) -> None:
        if manifest.get("versione") != "neural_model_v2_context":
        ...
        if architecture.get("name") != "neural_context_average_negative_sampling_v2":
            errors.append("Architettura modello errata.")

        if architecture.get("uses_multi_token_context") is not True:
            errors.append("Il modello non dichiara contesto multi-token.")

        if architecture.get("context_size", 0) < 2:
            errors.append("Context size non valido.")
        ...
        if data.get("train_examples_used", 0) <= 0:
            errors.append("Nessun esempio train usato.")

        if data.get("val_examples", 0) <= 0:
            errors.append("Nessun esempio validation.")

        if data.get("test_examples", 0) <= 0:
            errors.append("Nessun esempio test.")

    def _validate_weights(self, weights: Dict, manifest: Dict, errors: List[str]) -> None:
        if weights.get("versione") != "neural_model_v2_context_weights":
            errors.append("Versione pesi errata.")

        if weights.get("architecture") != "neural_context_average_negative_sampling_v2":
            errors.append("Architettura pesi errata.")
    ...
    def _validate_history(self, manifest: Dict, errors: List[str]) -> None:
        ...
        first_train = history[0].get("train_loss", 0)
        last_train = history[-1].get("train_loss", 0)

        if first_train <= 0 or last_train <= 0:
            errors.append("Train loss non valida.")

        if last_train >= first_train:
            errors.append("Train loss non è diminuita tra prima e ultima epoca.")
    ...
    def _validate_predictions(self, predictions: List[Dict], errors: List[str]) -> None:
        ...
        for item in predictions:
            if not item.get("context_tokens"):
                errors.append("Predizione senza context_tokens.")

            top_predictions = item.get("top_predictions", [])

            if not top_predictions:
                errors.append(f"Nessuna top_prediction per {item.get('context_tokens')}")
                continue
...
def build_report(errors: List[str], root: Path) -> str:
    ...
    return f"""# Validazione Neural Model V2 Context
...
def main() -> None:
    root = Path.cwd()
    report_path = root / "mini_llm" / "reports" / "validazione_neural_model_v2_context.md"

    validator = NeuralModelV2ContextValidator()
    ...
    if errors:
        print("ERRORE - Validazione Neural Model V2 Context fallita")
    ...
    print("OK - Validazione Neural Model V2 Context superata")
    print(f"Report: {report_path}")
