...
def formatta_fumetto_dialoghi(testo: str) -> str:
    """
    Rende leggibile il testo OCR di fumetti/balloon:
    - unisce righe spezzate
    - pulisce rumore
    - crea blocchi/dialoghi più naturali
    """
    testo = pulisci_testo(testo)
    ...
    correzioni = [
        ...
        (r"\bun mestiere faticoso\b", "un mestiere faticoso"),
        (r"\bnon lavorare\b", "non lavorare"),
        (r"\bdolce far niente\b", "dolce far niente"),
        (r"\bferie in corso\b", "ferie in corso"),
        (r"\bnon disturbare\b", "non disturbare"),
        (r"\bgià in pausa\b", "già in pausa"),
    ]

    for pattern, repl in correzioni:
        testo = re.sub(pattern, repl, testo, flags=re.I)

    # Torna a righe per ragionare meglio sul materiale OCR
    righe_raw = [
        r.strip()
        for r in testo.replace(" .", ".").replace(" !", "!").replace(" ?", "?").splitlines()
        if r.strip()
    ]
    ...
    def sembra_slogan(linea: str) -> bool:
        parole = linea.split()
        return len(parole) >= 2 and linea.upper() == linea

    def chiude_frase(linea: str) -> bool:
    ...
    for riga in righe:
        ...
        # Se la riga corrente non chiude il senso, continuo ad accodare
        if not chiude_frase(corrente):
            corrente += " " + riga
            continue

        # Se la riga nuova sembra un nuovo balloon / slogan, apro blocco nuovo
        if sembra_slogan(riga) or len(riga.split()) >= 4:
    ...
    for b in blocchi:
        ...
        # Riunisce slogan molto spezzati
        b = re.sub(r"\bLAVORARE STANCA NON LAVORARE RINGIOVANISCE\b", "Lavorare stanca, non lavorare ringiovanisce!", b, flags=re.I)
        b = re.sub(r"\bDIRETTORE DEL DOLCE FAR NIENTE\b", "Direttore del dolce far niente", b, flags=re.I)
        b = re.sub(r"\bFERIE IN CORSO\b", "Ferie in corso", b, flags=re.I)
        b = re.sub(r"\bNON DISTURBARE\b", "Non disturbare", b, flags=re.I)
        b = re.sub(r"\bOGGI PAUSA STRATEGICA\b", "Oggi pausa strategica.", b, flags=re.I)
        b = re.sub(r"\bPER FORZA ESSERE DISOCCUPATI TUTTO L'ANNO È UN MESTIERE FATICOSO\b", "Per forza essere disoccupati tutto l'anno è un mestiere faticoso.", b, flags=re.I)

        # Se è tutto maiuscolo ma non è lunghissimo, lo normalizzo
        if b.upper() == b and len(b.split()) <= 12:
            b = b.capitalize()

        blocchi_puliti.append(b)

    # Tenta di separare balloon/slogan dal testo di cartelli
    return "Dialoghi/balloon estratti:\n" + "\n".join(f"- {b}" for b in blocchi_puliti)
