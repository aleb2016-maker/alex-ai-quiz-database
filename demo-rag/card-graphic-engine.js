...
/* RAG_CV_CARD_QUALITY_FIX */
(function () {
  if (window.__ragCvCardQualityFixInstalled) return;
  window.__ragCvCardQualityFixInstalled = true;

  const BAD_CONCEPTS = new Set([
    "presso", "addetto", "pagina", "pag", "curriculum", "vitae",
    "email", "telefono", "indirizzo", "nascita", "nazionalita",
    "profilo", "contatti", "roma", "italiana"
  ]);
  ...
  function isContactCard(text) {
    return containsAny(text, [
      "email", "telefono", "indirizzo", "data e luogo di nascita",
      "nazionalita", "contatti", "libero.it", "@"
    ]);
  }

  function inferCvCard(text, index) {
    const normalized = normalizeText(text);

    if (containsAny(normalized, [
      "intelligenza artificiale", "prompt", "generativa", "ai",
      "strumenti digitali", "applicazioni android", "prototipi"
    ])) {
      return {
        materia: "ai",
        concetto: "competenze AI",
        fronte: "Scheda: Competenze digitali e AI"
      };
    }

    if (containsAny(normalized, [
      "creativita", "comunicazione", "pazienza", "adattamenti",
      "lavoro di gruppo", "pubblico", "competenze"
    ])) {
      return {
        materia: "generico",
        concetto: "competenze trasversali",
        fronte: "Scheda: Competenze trasversali"
      };
    }

    if (containsAny(normalized, [
      "esperienza", "lavoro", "addetto", "presso", "mansione",
      "azienda", "pulizie", "contratto"
    ])) {
      return {
        materia: "generico",
        concetto: "esperienza lavorativa",
        fronte: "Scheda: Esperienza lavorativa"
      };
    }

    if (containsAny(normalized, [
      "app", "android", "kotlin", "sviluppo", "progettazione",
      "software", "github", "database", "quiz"
    ])) {
      return {
        materia: "informatica",
        concetto: "progetti digitali",
        fronte: "Scheda: Progetti e applicazioni"
      };
    }

    if (containsAny(normalized, [
      "profilo", "creativo", "adattabile", "obiettivo",
      "candidato", "presentazione"
    ])) {
      return {
        materia: "generico",
        concetto: "profilo professionale",
        fronte: "Scheda: Profilo professionale"
      };
    }

    if (containsAny(normalized, [
      "formazione", "studio", "corso", "diploma", "competenze tecniche"
    ])) {
      return {
        materia: "generico",
        concetto: "formazione",
        fronte: "Scheda: Formazione e obiettivi"
      };
    }

    return {
      materia: "generico",
      concetto: "punto importante",
  ...
  function cleanCardText(text) {
    ...
    if (cleaned.length > 320) {
      cleaned = cleaned.slice(0, 320).replace(/\s+\S*$/, "") + "...";
    }

    return cleaned;
  }

  function improveCard(card, index) {
    const rawText = `${card.fronte || ""} ${card.retro || ""} ${card.concetto || ""}`;
    const currentConcept = normalizeText(card.concetto || card.fronte || "");

    const inferred = inferCvCard(rawText, index);

    if (
      BAD_CONCEPTS.has(currentConcept) ||
      normalizeText(card.fronte || "").startsWith("concetto chiave") ||
      containsAny(rawText, ["curriculum", "competenze", "esperienza", "profilo", "app", "intelligenza artificiale"])
    ) {
      card.materia = inferred.materia;
      card.concetto = inferred.concetto;
      card.fronte = inferred.fronte;
    }

    card.retro = cleanCardText(card.retro || rawText);

    if (!card.uso || card.uso.includes("Ripassa questo punto")) {
      card.uso = "Usa questa scheda per presentare meglio il curriculum o preparare un colloquio.";
    }
  ...
  function dedupeCards(cards) {
    ...
    cards.forEach(card => {
      const key = normalizeText(card.fronte || card.concetto || "");
  ...
  const previousMakeCards = window.makeCards;

  window.makeCards = function (rows, limit = 12) {
    let cards = [];

    if (typeof previousMakeCards === "function") {
      cards = previousMakeCards(rows, limit + 6);
    } else {
      cards = rows.slice(0, limit).map((row, index) => ({
        id: "RAG-CARD-" + String(index + 1).padStart(4, "0"),
        materia: "generico",
        concetto: row.concetto || "punto importante",
        fronte: "Scheda: " + (row.concetto || "Punto importante"),
        retro: row.spiegazione || "",
    ...
    cards = cards
      .filter(card => !isContactCard(`${card.fronte || ""} ${card.retro || ""}`))
      .map(improveCard);
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const leftPdfButton = document.getElementById("downloadCardsPdfButton");
    if (leftPdfButton) {
      leftPdfButton.remove();
    }

    const printButton = document.getElementById("printCardsButton");
    if (printButton) {
      printButton.textContent = "Salva / stampa card come PDF";
  ...
  document.addEventListener("click", function () {
    setTimeout(() => {
      const leftPdfButton = document.getElementById("downloadCardsPdfButton");
      if (leftPdfButton) leftPdfButton.remove();
