...
PHASE = "5.12E.1"
READY_LABEL = "QUALITY_REGISTRY_55_MOTORS_CONNECTED_V512E1"

REGISTRY_43_REPORT = REPORTS_DIR / "phase5_12d1_quality_registry_43_connected_v1.json"
QUIZ_REPORT = REPORTS_DIR / "phase5_12e_test_quiz_quality_motors_v1.json"
...
def _validate_registry_43(data: Optional[Dict[str, Any]]) -> List[str]:
    defects: List[str] = []

    if not data:
        return ["registry_43_report_missing_or_invalid"]
    ...
    if int(data.get("total_ready_connected_motors", -1)) != 43:
        defects.append(f"registry_43_total_not_43:{data.get('total_ready_connected_motors')}")
    if len(data.get("connected_motors") or []) != 43:
        defects.append(f"registry_43_connected_list_not_43:{len(data.get('connected_motors') or [])}")
    if data.get("defects"):
...
def _validate_quiz_report(data: Optional[Dict[str, Any]]) -> List[str]:
    defects: List[str] = []

    if not data:
        return ["quiz_report_missing_or_invalid"]

    if data.get("status") != "PASS":
        defects.append("quiz_status_not_pass")
    if data.get("approved") is not True:
        defects.append("quiz_not_approved")
    if data.get("ready_label") != "TEST_QUIZ_QUALITY_MOTORS_V512E_READY":
        defects.append("quiz_ready_label_missing")

    registry = data.get("registry") or {}
    if int(registry.get("total_motors", -1)) != 12:
        defects.append(f"quiz_total_motors_not_12:{registry.get('total_motors')}")

    if data.get("targeted_tests_passed") != data.get("targeted_tests_total"):
        defects.append("quiz_targeted_tests_not_all_passed")

    if (data.get("good_case") or {}).get("passed") is not True:
        defects.append("quiz_good_case_not_passed")

    if (data.get("good_case") or {}).get("blocking_issues", 999) != 0:
        defects.append("quiz_good_case_has_blocking_issues")
...
def _previous_motors(data: Dict[str, Any]) -> List[ConnectedMotor]:
    ...
    for m in data.get("connected_motors", []):
        rows.append(ConnectedMotor(
            ...
            source_report=str(m.get("source_report", _rel(REGISTRY_43_REPORT))),
...
def _new_quiz_motors(data: Dict[str, Any]) -> List[ConnectedMotor]:
    rows: List[ConnectedMotor] = []
    registry = data.get("registry") or {}
    ...
    for m in registry.get("motors", []):
        rows.append(ConnectedMotor(
            ...
            title=str(m.get("title")),
            source_phase="5.12E",
            source_report=_rel(QUIZ_REPORT),
            category="test_quiz_atomic_motor",
            status="READY_CONNECTED",
            confidence="HIGH",
            connection_type="registry_bridge_new_test_quiz_motor",
            areas=["test_quiz"],
        ))
...
def build_registry_bridge() -> RegistryBridgeResult:
    registry_43 = _load_json(REGISTRY_43_REPORT)
    quiz = _load_json(QUIZ_REPORT)

    defects: List[str] = []
    warnings: List[str] = []

    defects.extend(_validate_registry_43(registry_43))
    defects.extend(_validate_quiz_report(quiz))

    connected: List[ConnectedMotor] = []

    if registry_43:
        connected.extend(_previous_motors(registry_43))
    if quiz:
        connected.extend(_new_quiz_motors(quiz))
    ...
    previous_count = len([m for m in connected if m.source_phase != "5.12E"])
    quiz_count = len([m for m in connected if m.source_phase == "5.12E"])
    total = len(connected)

    if previous_count != 43:
        defects.append(f"previous_ready_connected_motors_not_43:{previous_count}")
    if quiz_count != 12:
        defects.append(f"connected_test_quiz_motors_not_12:{quiz_count}")
    if total != 55:
        defects.append(f"total_ready_connected_motors_not_55:{total}")
    ...
    remaining_atomic_controls = {
        ...
        "ready_connected_atomic_controls": 44,
        "still_to_recreate": 19,
        "still_to_verify": 1,
        "total_ready_connected_registry_motors": 55,
    }
    ...
    return RegistryBridgeResult(
        ...
        new_test_quiz_motors=quiz_count,
        ...
        scope_guard={
            ...
            "connected_existing_5_12e_motors": True,
