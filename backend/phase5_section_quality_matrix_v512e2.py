...
PHASE = "5.12E.2"
READY_LABEL = "SECTION_QUALITY_SELECTION_MATRIX_WITH_TEST_QUIZ_V512E2_READY"
...
def _classify_motors(connected: List[Dict[str, Any]]) -> Dict[str, List[str]]:
    ...
    for motor in connected:
        ...
        if source_phase == "5.12E" or "test_quiz" in category:
...
def build_section_matrix() -> SectionMatrixResult:
    data = _load_json(REGISTRY_55_REPORT)
    defects: List[str] = []
    warnings: List[str] = []
    connected: List[Dict[str, Any]] = []

    if not data:
        defects.append("registry_55_report_missing_or_invalid")
        registry_total = 0
    else:
        ...
        if registry_total != 55:
            defects.append(f"registry_55_total_not_55:{registry_total}")

        connected = list(data.get("connected_motors") or [])

        if len(connected) != 55:
            defects.append(f"registry_55_connected_motors_list_not_55:{len(connected)}")
        if data.get("defects"):
    ...
    card_active = _unique(foundation + textual + didactic + card_source)
    summary_active = _unique(foundation + textual + didactic + summary_source)
    study_active = _unique(foundation + textual + didactic)
    test_active = _unique(foundation + textual + didactic + quiz_specific)

    sections: Dict[str, Any] = {
        "card": {
            "description": "Card usa foundation + testuali universali + didattici + Card/Riassunto/Fonti. Non usa motori quiz specifici.",
            "active_motor_ids": card_active,
            "expected_active_count": len(card_active),
        },
        "summary": {
            "description": "Riassunto usa foundation + testuali universali + didattici + controlli riassunto/fonti.",
            "active_motor_ids": summary_active,
            "expected_active_count": len(summary_active),
        },
        "study_questions": {
            "description": "Domande studio usa foundation + testuali universali + didattici.",
            "active_motor_ids": study_active,
            "expected_active_count": len(study_active),
        },
        "test_quiz": {
            "description": "Test/Quiz usa foundation + testuali universali + didattici + motori specifici Test/Quiz.",
    ...
    if not set(quiz_specific).issubset(set(test_active)):
        defects.append("test_quiz_missing_test_quiz_specific_motors")
