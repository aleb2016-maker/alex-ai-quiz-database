...
PHASE = "5.12G.2"
READY_LABEL = "SECTION_QUALITY_SELECTION_MATRIX_WITH_CONTEXTUAL_DUPLICATES_V512G2_READY"

REGISTRY_65_REPORT = REPORTS_DIR / "phase5_12g1_quality_registry_65_connected_v1.json"
...
def _classify_motors(connected: List[Dict[str, Any]]) -> Dict[str, List[str]]:
    ...
    contextual_duplicates: List[str] = []
    ...
    for motor in connected:
        ...
        if source_phase == "5.12G" or "contextual_duplicate" in category:
            contextual_duplicates.append(mid)
        elif source_phase == "5.12F" or "advanced_language" in category:
            ...
            test_quiz.append(mid)
        elif num is not None and 45 <= num <= 50:
            contextual_duplicates.append(mid)
    ...
    return {
        ...
        "contextual_duplicate_universal": _unique(contextual_duplicates),
    }
...
def build_section_matrix() -> SectionMatrixResult:
    data = _load_json(REGISTRY_65_REPORT)
    defects: List[str] = []
    warnings: List[str] = []
    connected: List[Dict[str, Any]] = []

    if not data:
        defects.append("registry_65_report_missing_or_invalid")
        registry_total = 0
    else:
        registry_total = int(data.get("total_ready_connected_motors", 0))

        if data.get("status") != "PASS":
            defects.append("registry_65_status_not_pass")
        if data.get("approved") is not True:
            defects.append("registry_65_not_approved")
        if data.get("ready_label") != "QUALITY_REGISTRY_65_MOTORS_CONNECTED_V512G1":
            defects.append("registry_65_ready_label_missing")
        if registry_total != 65:
            defects.append(f"registry_65_total_not_65:{registry_total}")

        connected = list(data.get("connected_motors") or [])

        if len(connected) != 65:
            defects.append(f"registry_65_connected_motors_list_not_65:{len(connected)}")
        if data.get("defects"):
            defects.append("registry_65_has_defects")
    ...
    duplicates = groups["contextual_duplicate_universal"]

    duplicate_question_only = [mid for mid in duplicates if _motor_number(mid) == 48]
    duplicate_common = [mid for mid in duplicates if _motor_number(mid) != 48]
    ...
    expected = {
        ...
        "advanced_language": 4,
        "contextual_duplicates": 6,
        "contextual_duplicate_common": 5,
        "contextual_duplicate_question_only": 1,
    }
    ...
    actual = {
        ...
        "contextual_duplicates": len(duplicates),
        "contextual_duplicate_common": len(duplicate_common),
        "contextual_duplicate_question_only": len(duplicate_question_only),
    }
    ...
    card_active = _unique(foundation + textual + didactic + card_source + advanced + duplicate_common)
    summary_active = _unique(foundation + textual + didactic + summary_source + advanced + duplicate_common)
    study_active = _unique(foundation + textual + didactic + advanced + duplicate_common + duplicate_question_only)
    test_active = _unique(foundation + textual + didactic + quiz_specific + advanced + duplicate_common + duplicate_question_only)

    sections: Dict[str, Any] = {
        "card": {
            "description": "Card usa foundation + testuali + didattici + Card/Riassunto/Fonti + linguistici avanzati + duplicati contestuali comuni.",
            "active_motor_ids": card_active,
            "expected_active_count": 52,
        },
        "summary": {
            "description": "Riassunto usa foundation + testuali + didattici + riassunto/fonti + linguistici avanzati + duplicati contestuali comuni.",
            "active_motor_ids": summary_active,
            "expected_active_count": 47,
        },
        "study_questions": {
            "description": "Domande studio usa foundation + testuali + didattici + linguistici avanzati + duplicati contestuali, incluso qm_048.",
            "active_motor_ids": study_active,
            "expected_active_count": 43,
        },
        "test_quiz": {
            "description": "Test/Quiz usa foundation + testuali + didattici + Test/Quiz specifici + linguistici avanzati + duplicati contestuali, incluso qm_048.",
            "active_motor_ids": test_active,
            "expected_active_count": 55,
        },
    }
    ...
    for name, section in sections.items():
        ...
        if len(active) != int(section["expected_active_count"]):
            defects.append(f"{name}_active_count_not_{section['expected_active_count']}:{len(active)}")

    if set(duplicate_question_only) & set(card_active):
        defects.append("card_includes_qm_048_question_duplicate_motor")
    if set(duplicate_question_only) & set(summary_active):
        defects.append("summary_includes_qm_048_question_duplicate_motor")
    if not set(duplicate_question_only).issubset(set(study_active)):
        defects.append("study_missing_qm_048_question_duplicate_motor")
    if not set(duplicate_question_only).issubset(set(test_active)):
        defects.append("test_quiz_missing_qm_048_question_duplicate_motor")

    for section_name, active in {
        "card": card_active,
        "summary": summary_active,
        "study_questions": study_active,
        "test_quiz": test_active,
    }.items():
        if not set(advanced).issubset(set(active)):
            defects.append(f"{section_name}_missing_advanced_language_motors")
        if not set(duplicate_common).issubset(set(active)):
            defects.append(f"{section_name}_missing_common_contextual_duplicate_motors")
    ...
    return SectionMatrixResult(
        ...
        detected_groups={
            ...
            "contextual_duplicate_universal_count": len(duplicates),
            "contextual_duplicate_common_count": len(duplicate_common),
            "contextual_duplicate_question_only_count": len(duplicate_question_only),
            ...
            "contextual_duplicate_universal_ids": duplicates,
            "contextual_duplicate_common_ids": duplicate_common,
            "contextual_duplicate_question_only_ids": duplicate_question_only,
        },
        sections=sections,
        defects=defects,
        warnings=warnings,
        scope_guard={
            ...
            "changed_existing_65_motors": False,
            "changed_pipeline_5_11": False,
            "changed_ui_pdf_css_app": False,
            "matrix_only": True,
            "contextual_duplicates_added_to_card": True,
            "contextual_duplicates_added_to_summary": True,
            "contextual_duplicates_added_to_study_questions": True,
            "contextual_duplicates_added_to_test_quiz": True,
            "qm_048_only_for_study_and_test": True,
            "card_excludes_qm_048": True,
            "summary_excludes_qm_048": True,
